...
double get_frame_rate(unsigned int frame_rate_code) {
    switch (frame_rate_code) {
        ...
        case 7: return 59.94; 
        case 8: return 60;
        default: return 0; // Valeur non définie ou non prise en charge
    }
}
...
int mpeg2_header_sequence (mpeg2dec_t * mpeg2dec)
{
	...
	unsigned int frame_rate_code = buffer[3] & 0x0F; // les 4 derniers bits
    unsigned int cadence_ips = get_frame_rate(frame_rate_code); // Pseudo-fonction, suppose une table de correspondance
	
    if(cadence_ips > 0){
        sequence->frame_period = 27000000 / cadence_ips; // Calcul de frame_period
        printf("Frame Period: %u\n", sequence->frame_period); // Afficher ou assigner la valeur
    }
...
static int picture_coding_ext (mpeg2dec_t * mpeg2dec)
{
	...
	// Inside the picture_coding_ext function, after picture->flags has been set
	printf("Logging Picture Flags:\n");
	printf(" Progressive Sequence: %d\n", picture->flags & SEQ_FLAG_PROGRESSIVE_SEQUENCE ? 1 : 0);
	printf(" Progressive Frame: %d\n", picture->flags & PIC_FLAG_PROGRESSIVE_FRAME ? 1 : 0);
	printf(" Top Field First: %d\n", picture->flags & PIC_FLAG_TOP_FIELD_FIRST ? 1 : 0);
	printf(" Repeat First Field: %d\n", picture->flags & PIC_FLAG_REPEAT_FIRST_FIELD ? 1 : 0);
